...
class BirdsEyeViewCanvas(FigureCanvas):
    ...
    def setup_plot(self):
        """Initialize the plot."""
        self.ax.clear()
        self.ax.set_title("Bird's-Eye View - Click on a vehicle to select", 
    ...
    def plot_trajectories(self, current_time: Optional[float] = None):
        ...
        Args:
            current_time: If provided, show vehicle positions at this time
        """
        self.ax.clear()
        ...
        for veh_id, data in self.vehicle_data.items():
            ...
            line, = self.ax.plot(xs, ys, '-', alpha=0.3, linewidth=1, 
                                label=veh_id if len(self.vehicle_data) <= 20 else None)
            ...
            if current_time is not None:
                ...
                if len(time_diffs) > 0:
                    ...
                    marker = self.ax.scatter(xs[idx], ys[idx], s=100, 
                                           c=line.get_color(), 
                                           marker='o', 
                                           edgecolors='black', 
                                           linewidths=1.5,
                ...
                # If no time specified, show all positions
                marker = self.ax.scatter(xs, ys, s=50, 
                                       c=line.get_color(), 
                                       alpha=0.5,
                                       marker='o',
                                       zorder=5,
        ...
        # Add legend if not too many vehicles
        if len(self.vehicle_data) <= 10:
    ...
    def highlight_selected_vehicle(self):
        ...
        # Create highlight circle around the trajectory
        center_x = np.mean(xs)
        center_y = np.mean(ys)
    ...
    def on_click(self, event):
        ...
        for veh_id, data in self.vehicle_data.items():
            ...
            # Calculate distance to all positions of this vehicle
    ...
    def select_vehicle(self, vehicle_id: str):
        ...
        # Emit signal (will be connected in main window)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.init_ui()
        self.setWindowTitle("Bird's-Eye View Vehicle Selector & RDS-HQ Generator")
    ...
    def create_control_panel(self) -> QWidget:
        ...
        self.scene_idx_spin.setRange(0, 1000)
        self.scene_idx_spin.setValue(10)
        ...
        self.load_scene_btn = QPushButton("Load Scene & Generate Bird's-Eye View")
        ...
        self.selected_vehicle_label.setStyleSheet("font-weight: bold; color: red;")
        ...
        self.generate_rds_btn = QPushButton("Generate RDS-HQ for Selected Vehicle")
    ...
    def create_visualization_panel(self) -> QWidget:
        ...
        instructions = QLabel(
            "Instructions:\n"
            "1. Load InterHub data using the left panel\n"
            "2. Select a scene and time range, then click 'Load Scene'\n"
            "3. Click on any vehicle trajectory to select it\n"
            "4. Click 'Generate RDS-HQ' to create the view for selected vehicle"
        )
    ...
    def load_scene(self):
        """Load a scene and generate bird's-eye view."""
        if not self.bridge:
        ...
        try:
            ...
            # Generate FCD data temporarily
            output_dir = f"temp_scene_{scene_idx}"
            
            self.log("Generating trajectory data...")
            rds_path = self.bridge.generate_rds_hq_from_scene(
                scene_idx=scene_idx,
                time_start=time_start,
                time_end=time_end,
                output_dir=output_dir,
                streetview_retrieval=False,  # Don't need street view for BEV
                agent_clip_distance=self.agent_clip_spin.value(),
                camera_setting=self.camera_combo.currentText()
            )
            
            # Find FCD file
            fcd_files = list(Path(output_dir).rglob("*_fcd.xml"))
            if not fcd_files:
                raise FileNotFoundError("No FCD file generated")
            
            self.current_fcd_path = fcd_files[0]
            
            # Load trajectory data into canvas
            self.log("Loading trajectory visualization...")
            ...
            self.log(f"✓ Loaded {num_vehicles} vehicles")
            self.log("✓ Click on a vehicle to select it!")
    ...
    def update_selected_vehicle_info(self):
        """Update the selected vehicle information display."""
        ...
        displacement = np.sum(np.sqrt(np.diff(xs)**2 + np.diff(ys)**2))
...
Position Range:
  X: [{xs.min():.2f}, {xs.max():.2f}] m
  Y: [{ys.min():.2f}, {ys.max():.2f}] m
"""
        
        self.vehicle_info_text.setText(info)
        self.generate_rds_btn.setEnabled(True)
        
        self.log(f"Selected vehicle: {veh_id}")
    
    def generate_rds_hq(self):
        """Generate RDS-HQ for the selected vehicle."""
        if not self.canvas.selected_vehicle:
            QMessageBox.warning(self, "Warning", "Please select a vehicle first!")
            return
        
        if not self.current_scene_info:
            QMessageBox.warning(self, "Warning", "Please load a scene first!")
            return
        
        try:
            self.log(f"\n{'='*50}")
            self.log(f"Generating RDS-HQ for vehicle: {self.canvas.selected_vehicle}")
            self.log(f"{'='*50}")
            ...
            self.rds_thread = RDSHQGenerationThread(
                ...
                output_dir=f"rds_output_{self.canvas.selected_vehicle}",
            ...
            QMessageBox.critical(self, "Error", f"Failed to generate RDS-HQ:\n{str(e)}")
            self.generate_rds_btn.setEnabled(True)
            self.progress_bar.setVisible(False)
    
    def on_rds_generation_finished(self, rds_path: str):
        """Handle RDS-HQ generation completion."""
        self.progress_bar.setVisible(False)
        self.generate_rds_btn.setEnabled(True)
        
        self.log(f"✓ RDS-HQ generated successfully!")
        self.log(f"   Output: {rds_path}")
        
        QMessageBox.information(
            self,
            "Success",
            f"RDS-HQ data generated successfully!\n\nOutput location:\n{rds_path}"
        )
    
    def on_rds_generation_error(self, error_msg: str):
        """Handle RDS-HQ generation error."""
        self.progress_bar.setVisible(False)
        self.generate_rds_btn.setEnabled(True)
        ...
        QMessageBox.critical(self, "Error", f"RDS-HQ generation failed:\n{error_msg}")
...
def main():
    ...
    # Connect canvas selection to update info
